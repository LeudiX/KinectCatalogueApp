...
public class KinectManager : MonoBehaviour 
{
	...
	protected static KinectManager instance;
...
//				dwFlags |= KinectInterop.FrameSource.TypeAudio;
			...
			if (sensorData == null)
			{
				if(sensorInterfaces == null || sensorInterfaces.Count == 0)
					throw new Exception("El sensor no ha sido encontrado. Por favor asegurese de haber instalado la SDK y verifique si el dispositivo está conectado .");
				else
					throw new Exception("Apertura del Sensor principal fallida.");
			}
		...
		{
			string message = ex.Message + " no puede ser cargado. Por favor, revise si tiene instalada la SDK .";
...
//Desahabilito este fragmento de codigo porque no me permite cambiar entre escenas manteniendo la interaccion con Kinect 2
...
//#endif
		...
		if(calibrationText != null)
		{
			calibrationText.text = "Esperando Usuarios...";
		}
		
		Debug.Log("Esperando Usuarios...");
	}
...
//					}
	...
	void Update() 
	{
		if(kinectInitialized)
		{
			//Añadido por mi para verificar la continuidad de trabajo con el Kinect 2 durante el cambio de escenas
			instance = gameObject.GetComponent<KinectManager>();
